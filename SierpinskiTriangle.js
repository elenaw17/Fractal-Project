...
*/
function createTrianglePointArray(centerX, centerY, radius) {
    var angle = Math.PI / -6;
    var xCoord;
    var yCoord;
...
function calculateNewCenters(centerX, centerY, radius) {
    ...
    for (var i = 0; i < 3; i++) {
        xCoord = centerX + (Math.cos(angle) * radius / 2);
        yCoord = centerY - (Math.sin(angle) * radius / 2);
        // y coord subtracts radius to account for reversed y coordinates
        pointArray.push([xCoord, yCoord]);
        angle = angle + 2/3 * Math.PI;
    }
...
radiusDivisor, completedIterations, totalIterations) {
    ...
    if (completedIterations === 0) {
        pointArray = [];
        singleIterationArray = createTrianglePointArray(xCenter, yCenter, radius);
